"""Build a production-grade Vietnamese Manim Shorts about the architecture of mathematical space.
...
SCENE_FILE = PROJECT / "space_architecture.py"
CONFIG_FILE = PROJECT / "config.json"
ASS_FILE = PROJECT / "subtitles.ass"
FINAL_MP4 = PROJECT / "space_architecture_voice_sfx.mp4"
POLISHED_MP4 = PROJECT / "space_architecture_voice_sfx_polished.mp4"
MUSIC_MIX_MP4 = PROJECT / "space_architecture_reference_music_mix.mp4"
...
SEGMENTS: list[dict[str, Any]] = [
    {
        "id": "01_hook",
        "type": "hook",
        "narration": "Mỗi con số sống trong một không gian. Không gian đó xây từ đâu?",
    },
    {
        "id": "02_origin_vector",
        "type": "origin_vector",
        "narration": "Đầu tiên là một điểm gốc. Từ đó, một vector bước ra.",
    },
    {
        "id": "03_basis",
        "type": "basis",
        "narration": "x, y không phải vị trí. Chúng là cách kéo giãn i-hát và j-hát.",
    },
    {
        "id": "04_span",
        "type": "span",
        "narration": "Cộng hai vector đã scale: tổ hợp tuyến tính. Mọi điểm đạt được gọi là span.",
    },
    {
        "id": "05_basis_alt",
        "type": "basis_alt",
        "narration": "i-hát, j-hát là một bộ cơ sở. Cặp vector khác vẫn xây được cùng mặt phẳng.",
    },
    {
        "id": "06_dimension",
        "type": "dimension",
        "narration": "Số vector tối thiểu cần thiết là số chiều. Thêm k-hát, không gian thành 3D.",
    },
    {
        "id": "07_higher_dim",
        "type": "higher_dim",
        "narration": "Không dừng ở 3D. Dữ liệu AI sống trong hàng trăm chiều.",
    },
    {
        "id": "08_outro",
        "type": "outro",
        "narration": "Đó là kiến trúc không gian. Hiểu nó, bạn thấy toán học ở khắp nơi.",
    },
]
...
def generate_audio() -> list[dict[str, Any]]:
    ...
    for seg in SEGMENTS:
        ...
        if not out.exists():
            ...
            tts_tools.text_to_speech(seg["narration"], str(tmp_wav), voice=VOICE)
            # edge-tts may output WAV even for .mp3; force a real MP3 here
            convert = run(
                [
                    "ffmpeg",
                    "-y",
            ...
            if convert.returncode != 0:
                raise RuntimeError(f"TTS conversion failed for {seg['id']}: {convert.stderr}\n{convert.stdout}")
...
def generate_ass(segments: list[dict[str, Any]]) -> None:
    header = """[Script Info]
Title: Space Architecture Short Vietnamese Subtitles
ScriptType: v4.00+
PlayResX: 1080
PlayResY: 1920
...
def render_manim() -> Path:
    # Re-use an already-rendered video when the source has not changed.
    existing = (
        PROJECT
        / "media"
        / "videos"
        / "space_architecture"
        / "1920p30"
        / "SpaceArchitectureShort.mp4"
    )
    if existing.exists():
        print("  reusing existing Manim render...")
        return existing

    print("Rendering Manim space architecture scene...")
    result = run(
        [
            str(MANIM),
            "-q",
    ...
    if not out.exists():
        matches = list((PROJECT / "media" / "videos" / "space_architecture").rglob("SpaceArchitectureShort.mp4"))
        if matches:
            out = matches[0]
        else:
            raise RuntimeError("Manim did not produce SpaceArchitectureShort.mp4")
    return out
...
def generate_sfx_library() -> dict[str, Path]:
    ...
    make_sfx(
        ...
        0.12,
        sfx_dir / "math_pulse.wav",
    )
    sfx["math_pulse"] = sfx_dir / "math_pulse.wav"
    ...
    sfx["tone_hit"] = sfx_dir / "interference_hit.wav"
...
def build_sfx_timeline(sfx: dict[str, Path], duration: float) -> Path:
    ...
    timeline = [
        ...
        ("digital_snap", 1.0, 0.08, 0.7),
        ("math_pulse", 6.0, 0.12, 0.5),
        ("switch_click", 10.0, 0.05, 0.5),
        ("math_pulse", 17.0, 0.12, 0.6),
        ("sweep_snap", 23.0, 0.5, 0.7),
        ("switch_click", 27.0, 0.05, 0.5),
        ("math_pulse", 31.0, 0.12, 0.5),
        ("riser", 36.0, 2.5, 0.3),
        ("sweep_snap", 41.0, 0.5, 0.6),
        ("math_pulse", 49.0, 0.12, 0.5),
        ("math_pulse", 54.0, 0.12, 0.5),
        ("digital_snap", 60.0, 0.08, 0.5),
        ("sweep_snap", 66.0, 0.5, 0.7),
        ("math_pulse", 72.0, 0.12, 0.5),
        ("digital_snap", 82.0, 0.08, 0.5),
    ]
    inputs: list[str] = []
...
def main() -> None:
    print("=" * 60)
    print("Building Space Architecture Short")
    print("=" * 60)
    ...
    segments = pad_segments(segments, target=85.0)
    ...
    # Make the polished version the final deliverable
    shutil.copy(polished, FINAL_MP4)
    polished = FINAL_MP4
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "audio":
        segs = generate_audio()
        segs = pad_segments(segs, target=85.0)
        write_config(segs)
        generate_ass(segs)
