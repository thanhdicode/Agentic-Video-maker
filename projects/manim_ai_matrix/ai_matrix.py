...
class NeuralNetMatrixMath(MovingCameraScene):
    ...
    def _vec_label(self, text, color=WHITE, font_size=34, buff=0.3):
        """Place a label above and to the right of the current active vector tip."""
        label = MathTex(text, color=color, font_size=font_size)
        tip = self.active_vec.get_end()
        label.next_to(tip, RIGHT, buff=buff)
        label.shift(UP * 0.5)
    ...
    def _seg_input(self, seg, dur):
        ...
        # Grow the input vector, then label it
        self.play(GrowArrow(self.x_vec), run_time=1.0)
        self.x_label = self._vec_label("x", color=YELLOW)
        self.play(FadeIn(self.x_label), run_time=0.4)

        desc = MathTex(r"x = \begin{bmatrix} 1.2 \\ -0.8 \end{bmatrix}", color=WHITE, font_size=28)
        desc.to_edge(RIGHT, buff=0.8)
        desc.shift(UP * 2.4)
        desc.add_background_rectangle(color=BLACK, opacity=0.7)
        self.play(FadeIn(desc), run_time=0.6)

        self.wait(max(0.6, dur - 0.8 - 1.0 - 0.4 - 0.6))
    ...
    def _seg_matrix_intro(self, seg, dur):
        ...
        self.w_tex.to_edge(RIGHT, buff=0.4)
        self.w_tex.shift(UP * 2.2)
        self.w_tex.add_background_rectangle(color=BLACK, opacity=0.7)
    ...
    def _seg_layer1(self, seg, dur):
        # Move labels and basis vectors aside for the main transform
        to_fade = [self.x_label]
        if hasattr(self, "basis_group"):
            to_fade.append(self.basis_group)
        self.play(*[FadeOut(m) for m in to_fade], run_time=0.3)
        ...
        # Cinematic camera zoom on the transformed vector, then reset
        self.play(self.camera.frame.animate.scale(1.15).move_to(self.active_vec.get_end()), run_time=0.6)
        self.play(self.camera.frame.animate.scale(1 / 1.15).move_to(ORIGIN), run_time=0.6)

        self.wait(max(0.2, dur - 0.3 - 1.8 - 0.4 - 1.2))
    ...
    def _seg_bias(self, seg, dur):
        ...
        self.b_tex.to_edge(RIGHT, buff=0.4)
        self.b_tex.shift(UP * 1.5)
        self.b_tex.add_background_rectangle(color=BLACK, opacity=0.7)
        ...
        self.eq.add_background_rectangle(color=BLACK, opacity=0.7)
    ...
    def _seg_relu(self, seg, dur):
        ...
        self.relu_axes.to_edge(RIGHT, buff=0.4)
        self.relu_axes.shift(DOWN * 2.5)
        ...
        self.relu_label.next_to(self.relu_axes, UP, buff=0.3)
        self.relu_label.add_background_rectangle(color=BLACK, opacity=0.7)
        ...
        self.play(self.z_vec.animate.put_start_and_end_on(ORIGIN, h_point), run_time=0.8)
        self.z_vec.set_color(GREEN)
        self.h_vec = self.z_vec
        self.active_vec = self.h_vec

        self.h_label = self._vec_label(r"h = \text{ReLU}(z)", color=GREEN)
        self.play(FadeIn(self.h_label), run_time=0.4)

        self.wait(max(0.5, dur - 0.3 - 0.8 - 0.8 - 0.4))

    def _seg_layer2(self, seg, dur):
        self.play(FadeOut(self.h_label), run_time=0.3)

        # Clear first-layer annotations and ReLU plot to reduce right-side clutter
        to_clear = [self.w_tex, self.b_tex, self.eq]
        if hasattr(self, "relu_axes"):
            to_clear.extend([self.relu_axes, self.relu_graph, self.relu_label])
        self.play(*[FadeOut(m) for m in to_clear], run_time=0.3)

        # V matrix appears
        self.v_tex = self._matrix_tex("V", W2, color=PURPLE)
        self.v_tex.to_edge(RIGHT, buff=0.4)
        self.v_tex.shift(UP * 2.2)
        self.v_tex.add_background_rectangle(color=BLACK, opacity=0.7)
        self.play(FadeIn(self.v_tex), run_time=0.5)

        # Apply V to h vector, then add b2
        ...
        self.play(self.h_vec.animate.put_start_and_end_on(ORIGIN, y_point), run_time=0.8)
        self.h_vec.set_color(PURPLE)
        self.y_vec = self.h_vec
        self.active_vec = self.y_vec

        # Camera zoom on the output vector, then reset before the label
        self.play(self.camera.frame.animate.scale(1.15).move_to(self.active_vec.get_end()), run_time=0.6)
        self.play(self.camera.frame.animate.scale(1 / 1.15).move_to(ORIGIN), run_time=0.6)
        ...
        self.wait(max(0.2, dur - 0.3 - 0.3 - 0.5 - 1.6 - 0.8 - 1.2 - 0.4))
